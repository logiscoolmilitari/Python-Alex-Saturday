...
while not found_secret:
    answer = str(input("Enter a string: "))
    answer_length = len(answer)
    if answer_length >= 8 and answer_length < 10:
        print("You passed the first level!")
        a_random_number_from_user = int(input("Enter a number: "))
        while a_random_number_from_user >= -2:
            a_random_number_from_user = int(input("Enter a number: "))
        if a_random_number_from_user < -8 and (a_random_number_from_user * 5) == -45:
