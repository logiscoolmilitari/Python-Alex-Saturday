...
while not found_secret:
    answer = input("Enter a string: ")      # animal
    answer_length = len(answer)
    if answer_length >= 6 and answer_length < 9:
        print("You passed the first level!")
        a_random_number_from_user = int(input("Enter a number: "))
        while a_random_number_from_user >= -1:
            a_random_number_from_user = int(input("Enter a number: "))
        if a_random_number_from_user < -11 and (a_random_number_from_user * 4) == -48:  # -12
            print("You got the right number!")
            a_big_step = input("Enter the correct answer: ")   # dcba
            is_correct = True
